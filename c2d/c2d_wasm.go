// Code generated by generate_canvas_api_wrapper - DO NOT EDIT.
//
// A typed wrapper over CanvasRenderingContext2D, printed from
// TypeScript's own DOM library by walking the type with the TypeScript
// compiler API (../generate_canvas_api_wrapper). The generator needs
// yarn and tsc, and the DOM does not move, so this file is committed
// and the generator is not in the build graph: regenerate it by hand
// when the browser grows a method worth having.
